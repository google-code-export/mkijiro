...
//Globals
unsigned char *NPRVER="nitePRmod 20110529";
...
#ifdef _SLIM_
#define RAMTEMP  0x8A001000 //fat moviezone,slim user use extraram 0x8A001000
#define SAVETEMP 0x8A001000 //fat moviezone,slim user use extraram 0x8A001000
#define SRMAX    0x00200000 //*8
#else
          ...
          if(pad.Buttons & PSP_CTRL_CROSS)
          {
            ...
            else if(extSelected[0] == 1)
            {
              ...
              if(searchNo == 0)
              {
                ...
                fd=sceIoOpen(buffer, PSP_O_WRONLY | PSP_O_CREAT | PSP_O_APPEND, 0777);
                if(fd>0)
                {
                  //Write out the searcHistory[0] type
                 switch(searchHistory[0].flags & FLAG_DWORD)
                {
				case FLAG_DWORD: miscType=4;//sceIoWrite(fd,&mode,1);
				DumpByte=8;cmpval=searchHistory[0].hakVal;break;
				case FLAG_WORD:	miscType=2;//sceIoWrite(fd,&mode,1);
				DumpByte=6;cmpval=searchHistory[0].hakVal&0xFFFF;break;
				case FLAG_BYTE:	miscType=1;//sceIoWrite(fd,&mode,1);
				DumpByte=5;cmpval=searchHistory[0].hakVal&0xFF;break;
                }
                 ...
                 while(counter<0x4A000000){
        			...
        			lineClear(33);
        			sprintf(buffer, "Task = %02d%%; () = Hold to Abort", (counter-0x48800000)/0x3D70A); //1800000€64
        			pspDebugScreenPuts(buffer);
                    ...
                    switch(searchHistory[0].flags & FLAG_DWORD)
                  	{
                    	case FLAG_DWORD:
	                  while(subCounter<SRMAX)
    	             	{
    	             		
	                	if(counter>=0x4A000000) goto RDUMP;
	                	
                      	if(*((unsigned int*)(counter)) == cmpval)
                        {
                          //Add it
		                  searchResultCounter++;
			 				*(unsigned int *)(RAMTEMP+0+8*subCounter)=counter;
			 				*(unsigned int *)(RAMTEMP+4+8*subCounter)=searchHistory[0].hakVal;
                          subCounter++;
                        }
                      	counter+=4;
                      }
                    	...
                    	case FLAG_WORD:
	                  while(12*subCounter<16*SRMAX)//4byte alignement
    	             	{
    	             		
	                	if(counter>=0x4A000000) goto RDUMP;
	                	
                      	if(*((unsigned short*)(counter)) == cmpval)
                        {
                          //Add it
		                  searchResultCounter++;
							__asm__  (
								...
								"swr %0,0(%1)\n"
								:"=r"(counter)
								:"r"(RAMTEMP+6*subCounter)
								);
			 				*(unsigned short *)(RAMTEMP+4+6*subCounter)=searchHistory[0].hakVal;
                          subCounter++;
                        }
                      	counter+=2;
                      }
                    	...
                    	case FLAG_BYTE:
	                  while(20*subCounter<32*SRMAX)//4byte alignemnt
    	             	{
    	             		
	                	if(counter>=0x4A000000) goto RDUMP;
	                	
                      	if(*((unsigned char*)(counter)) == cmpval)
                        {
                          //Add it
		                  searchResultCounter++;
							__asm__  (
								...
								"swr %0,0(%1)\n"
								:"=r"(counter)
								:"r"(RAMTEMP+5*subCounter)
								);
			 				*(unsigned char *)(RAMTEMP+4+5*subCounter)=searchHistory[0].hakVal;
                          subCounter++;
                        }
                      	counter++;
                      }
                  ...
                  RDUMP:
                  sceIoWrite(fd, RAMTEMP, DumpByte*subCounter);//dump must be 4byte,other byte too slow...
                  subCounter=0;
                 }
				 sceIoClose(fd);
