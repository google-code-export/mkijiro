...
Public Class Form1
    ...
    '巨大UTF16LEから各エンコードに変換
    Private Function utf16vstable(ByVal output As String, ByVal enc As Integer) As Boolean
    ...
    'sjis/euc/gbk/jisからUTF8直変換
    Private Function encvsutf8table(ByVal output As String, ByVal enc As Integer) As Boolean
                ...
                If ENCODE.SelectedIndex = 11 Then
                    ...
                    If jis.Checked = True Then
                        ...
                        If selmode.SelectedIndex = 0 Then
                            ...
                            'sjis2004
                        ElseIf selmode.SelectedIndex = 2 Then
                            If (((c1 Xor &H20) + &H5F) And &HFF) < &H3C AndAlso ((c2 + &HC0) And &HFF) < 189 AndAlso c2 <> &H7F Then
                                sjis = (c1 << 8) + c2
                                hantei = 0
                            ...
                            'euc
                        ElseIf selmode.SelectedIndex = 1 Then
    ...
    'unicodeコンソーシアムのテーブル SJIS JIS UNICODE の順番
    Private Function unicodetable(ByVal output As String, ByVal enc As Integer) As Boolean
    ...
    '特大utf32からjisx0213に変換　
    Private Function uni_jisx0213table(ByVal output As String, ByVal enc As Integer) As Boolean
    ...
    'SJIS,EUC,GBK <>　UTF8の変換　ブルーフォース方式
    Private Function utf8vsenc(ByVal output As String, ByVal enc As Integer) As Boolean
    ...
    'テーブル作成
    Private Sub MKTBL(sender As System.Object, e As System.EventArgs) Handles MAKETABLE.Click
        Try
            ...
            If m.Success Then
                ...
                If (ENCODE.SelectedIndex >= 3 AndAlso ENCODE.SelectedIndex <= 5) Then
                    utf16vstable(output, enc)

                    'HTML
                ElseIf ENCODE.SelectedIndex = 12 Then
                    tsvtable_html(output, enc)

                    'SJIS,GBK,EUC VS テーブル,フォントロード使い回し
                ElseIf ENCODE.SelectedIndex >= 8 Then
                    encvsutf8table(output, enc)

                    'てきすとからいっぱい
                ElseIf ENCODE.SelectedIndex = 6 Then
                    unicodetable(output, enc)

                    'jisx0213
                ElseIf ENCODE.SelectedIndex = 7 Then
                    uni_jisx0213table(output, enc)


                ElseIf File.Exists(output) Then
                    utf8vsenc(output, enc)
    ...
    '各テーブルのテスト
    Private Sub TESTER(sender As System.Object, e As System.EventArgs) Handles TABLETEST.Click
        Try
            Dim mm As New Regex("CP\d+", RegexOptions.ECMAScript)
            Dim m As Match = mm.Match(ENCODE.Text)
            If m.Success Then
