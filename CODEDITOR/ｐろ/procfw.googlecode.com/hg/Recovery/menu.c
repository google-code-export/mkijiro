...
{
	sprintf(buf, "%s\t%s", g_messages[FAKE_REGION],get_fake_region_name(g_config.fakeregion));
...
{
sprintf(buf, "%s\t%s", g_messages[ISO_MODE], get_iso_name(g_config.umdmode));
...
{
	sprintf(buf, "%s\t%s", g_messages[XMB_USBDEVICE], get_usbdevice_name(g_config.usbdevice));
...
{
	sprintf(buf, "%s\t%s", g_messages[USB_CHARGE], get_bool_name(g_config.usbcharge));
...
{
	sprintf(buf, "%s\t%s", "Language", get_language_name(g_config.language));
...
{
	sprintf(buf, "%s\t%s", g_messages[SLIM_COLOR], get_bool_name(g_config.slimcolor));
...
{
	sprintf(buf, "%s\t%s", g_messages[HTMLVIEWER_CUSTOM_SAVE_LOCATION],get_bool_name(g_config.htmlviewer_custom_save_location));
...
{
	sprintf(buf, "%s\t%s", g_messages[HIDE_MAC], get_bool_name(g_config.machidden));
...
{
	sprintf(buf, "%s\t%s", g_messages[SKIP_SONY_LOGO], get_bool_name(g_config.skiplogo));
...
{
	sprintf(buf, "%s\t%s", g_messages[SKIP_GAME_BOOT], get_bool_name(g_config.skipgameboot));
...
{
	sprintf(buf, "%s\t%s", g_messages[HIDE_PIC], get_bool_name(g_config.hidepic));
...
{
	sprintf(buf, "%s\t%s", g_messages[FLASH_PROTECT], get_bool_name(g_config.flashprot));
...
{
	sprintf(buf, "%s\t%s", g_messages[USE_VERSION_TXT], get_bool_name(g_config.useversion));
...
{
	sprintf(buf, "%s\t%s", g_messages[USE_USBVERSION_TXT], get_bool_name(g_config.usbversion));
...
{
	sprintf(buf, "%s\t%s", g_messages[USE_CUSTOM_UPDATE_SERVER], get_bool_name(g_config.useownupdate));
...
{
	sprintf(buf, "%s\t%s", g_messages[PREVENT_HIB_DEL], get_bool_name(g_config.hibblock));
...
{
	sprintf(buf, "%s\t%s", g_messages[XMB_PLUGIN], get_bool_name(g_config.plugvsh));
...
{
	sprintf(buf, "%s\t%s", g_messages[GAME_PLUGIN], get_bool_name(g_config.pluggame));
...
{
	sprintf(buf, "%s\t%s", g_messages[POPS_PLUGIN], get_bool_name(g_config.plugpop));
...
{
	sprintf(buf, "%s\t%s", g_messages[USE_NODRM_ENGINE], get_bool_name(g_config.usenodrm));
...
{
	sprintf(buf, "%s\t%s", g_messages[BLOCK_ANALOG_INPUT], get_bool_name(g_config.noanalog));
...
{
	sprintf(buf, "%s\t%s", g_messages[OLD_PLUGINS_SUPPORT], get_bool_name(g_config.oldplugin));
...
{
	sprintf(buf, "%s\t%s", g_messages[ISO_CACHE], get_bool_name(g_config.iso_cache));
...
{
	sprintf(buf, "%s\t%3d", g_messages[ISO_CACHE_TOTAL_SIZE], g_config.iso_cache_total_size);
...
{
	sprintf(buf, "%s\t%3d", g_messages[ISO_CACHE_NUMBER], g_config.iso_cache_num);
...
{
	sprintf(buf, "%s\t%s", g_messages[ISO_CACHE_POLICY], get_cache_policy_name(g_config.iso_cache_policy));
...
{
	sprintf(buf, "%s\t%s", g_messages[ALLOW_NON_LATIN1_ISO_FILENAME], get_bool_name(g_config.chn_iso));
...
static int display_msspeed(struct MenuEntry* entry, char *buf, int size)
{
	...
	sprintf(buf, "%s\t%s", g_messages[MSSPEED_UP], speedstr);
...
{
	sprintf(buf, "%s\t%s", g_messages[HIDE_CFW_DIRS], get_bool_name(g_config.hide_cfw_dirs));
...
{
	sprintf(buf, "%s\t%s", g_messages[RETAIL_HIGH_MEMORY], get_bool_name(g_config.retail_high_memory));
