...
/*
int encode_utf16_conv_noram(const unsigned char *ucs, unsigned char *cjk,int len)
{
	...
	{
		ucs4_t u = 0x1FFF;
		int p = utf16_mbtowc(&u, ucs + i, len - i);
		if(p < 0)
			break;
		if(u > 0xFFFF)
			u = 0x1FFF;
...
int SJIS_UTF8SJIS(unsigned char *msg, int len){
	...
	if(filename_encode==UTF8){
		...
		else if(((u8)((c1^0x20)- 0xA1) < (u8)0x3C) && (c2>=0x40)){//SJIS判定
